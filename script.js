setTimeout(() => {
  ...
  const adicionaClasseAosCartoes = (address, classe) =>
    address.parent().removeClass().addClass(classe);

  const backgroundcolor = (address, cor) =>
  ...
  const pintaAnalogicasTemposDeCiclo = () => {
    for (let n in entradaAnalogica) {
      entradaAnalogica[n] = Math.abs(
        entradaAnalogica[n] + parseInt((Math.random() - 0.5) * 10)
      );
    ...
    if (todosIDs.length > 0)
      for (let n in armazenaBorda) {
        if (armazenaBorda[n] != IO[n] && todosIDs[n][0] == "I")
          arrayApagar.push(new objGravaBorda(IO[n], todosIDs[n]));
      }
  ...
  const selecionaValor = (nome, valor) =>
